...
class GUI_Equation():
    def __init__(self, parent_frame, controller, mode="add", selected=None):
        ...
        # Etiqueta "Ecuación"
        equation_label = ctk.CTkLabel(input_frame, text="Ecuación",fg_color="transparent", text_color="#FFFFFF", font=("Inter", 12))
        equation_label.pack(side="top", fill="x", padx=10, pady=2)

        # Entry para la ecuación
        self.text_entry_equation_window = ctk.CTkEntry(input_frame)
        self.text_entry_equation_window.pack(side="top", fill="x", padx=10, pady=2)

        # Etiqueta "Variables"
        variables_label = ctk.CTkLabel(input_frame, text="Variables", fg_color="transparent", text_color="#FFFFFF", font=("Inter", 12))
        variables_label.pack(side="top", fill="x", padx=10, pady=2)

        # Entry para las variables
        self.text_entry_var = ctk.CTkEntry(input_frame)
        self.text_entry_var.pack(side="top", fill="x", padx=10, pady=2)

        # Etiqueta "Constantes"
        constants_label = ctk.CTkLabel(input_frame, text="Constantes", fg_color="transparent", text_color="#FFFFFF", font=("Inter", 12))
        constants_label.pack(side="top", fill="x", padx=10, pady=2)

        # Entry para las constantes
        self.text_entry_constants = ctk.CTkEntry(input_frame)
        self.text_entry_constants.pack(side="top", fill="x", padx=10, pady=2)

        # Frame para el canvas de LaTeX
        latex_frame = ctk.CTkFrame(self.main_container, corner_radius=10)
        latex_frame.pack(side="top", fill="both", expand=True, pady=10)
    ...
    def plot_text_equation(self, parent_frame):
        ...
        self.latex_canvas.get_tk_widget().pack(expand=True, fill="both")
